...
class DonorGraph:
    ...
    def import_data(self, contributions_df: pd.DataFrame, filers_df: pd.DataFrame):
        ...
        try:
            with self.driver.session() as session:
                ...
                filers_df[['MUNICIPALITY', 'SUBDIVISION']] = filers_df['MUNICIPALITY_DESC, SUBDIVISION_DESC'].str.split(
                    ',', expand=True)
    ...
    @logger.catch
    def import_data_from_model(self, contributions_df: pd.DataFrame, filers_df: pd.DataFrame):
    ...
    @logger.catch
    def import_data_from_query_builder(self, contributions_df: pd.DataFrame, filers_df: pd.DataFrame):
        ...
        try:
            ...
            for i, row in contributions_df.iterrows():
                try:
                    donor_id = self.generate_donor_id(row)
                    donor_type = 'Organization' if pd.isna(row['FLNG_ENT_FIRST_NAME']) else 'Individual'
                    donor_name = self.clean_name(row['FLNG_ENT_NAME']) if donor_type == 'Organization' \
                    ...
                    (create()
                     ...
                     .match()
                     .node(labels="Filer", filer_id=str(row['FILER_ID']), variable="f")
                     .create()
                    ...
                    logger.error(f"Error processing contribution {row['TRANS_NUMBER']}: {str(e)}")
                    raise
